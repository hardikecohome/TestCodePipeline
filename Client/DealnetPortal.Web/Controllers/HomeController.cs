...
namespace DealnetPortal.Web.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> GetWorkItems(bool? completedOnly)
        {
            ...
            var contractsShortInfo = (await _contractServiceAgent.GetContractsShortInfo()).OrderByDescending(x => x.IsNewlyCreated)
                .ThenByDescending(x => string.IsNullOrEmpty(x.TransactionId))
                .ThenByDescending(x => x.LastUpdateTime)
                .ToList();
            var contractsShortInfoVms = AutoMapper.Mapper.Map<IList<DealItemOverviewViewModel>>(contractsShortInfo);
